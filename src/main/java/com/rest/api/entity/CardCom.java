...
@Table(name = "tbl_cardcom") // 'tbl_infobill' 테이블과 매핑됨을 명시
public class CardCom {
    //  pk생성전략을 DB에 위임한다는 의미입니다.
    //  mysql로 보면 pk 필드를 auto_increment로 설정해 놓은 경우로 보면 됩니다.
    @Id // primaryKey임을 알립니다.
    @GeneratedValue(strategy = GenerationType.IDENTITY) private long card_id;
    @Column(nullable = false, length =  500) private String full_text;
    @Column(nullable = true, length =   10) private String recv_rslt;
